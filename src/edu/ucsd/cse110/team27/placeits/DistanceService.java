...
public class DistanceService extends Service {
	...
	class LocationRangeTask extends AsyncTask<Object, Integer, List<PlaceIt>> {
		...
		@Override
		protected void onPostExecute(List<PlaceIt> result) {
			for (PlaceIt placeIt : result) {
				printNotification(placeIt.getTitle(), placeIt.getDescription(),
						placeIt);
			}
		}
	};
	...
	private LatLng getCurrentPosition() {
		...
		// return new LatLng(MapActivity.lastLocation.getLatitude(),
		// MapActivity.lastLocation.getLongitude());
	...
	private void printNotification(String title, String text, PlaceIt object) {
		...
		resInt.putExtra(PlaceIt.PLACEIT_POS_KEY, ActivePlaceIts.getInstance()
				.getPosition(object));
		resInt.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
				| Intent.FLAG_ACTIVITY_CLEAR_TASK);

		PendingIntent resPendInt = PendingIntent.getActivity(this, 0, resInt,
