...
public class distanceService extends Service {
	...
	public void onCreate(){ 
		...
		activeList = ActivePlaceIts.getInstance(null).getList();
		// create new runnable
		distanceNoti = new Runnable(){
			...
			public void run() {
				...
				for(int i = 0; i < activeList.size(); i++){
					...
					placeItLoc = convertLatLoc(activeList.get(i).getLocation());
	...
	public void onStart(Intent intent, int startid) {
		...
		ActivePlaceIts.getInstance(null).getList();
		distanceNoti.run();
    ...
    private void printNotification(String title, String text, PlaceIt object) {
    	...
    	resInt.putExtra(PlaceIt.PLACEIT_POS_KEY, ActivePlaceIts.getInstance(null).getPosition(object));
